...
const QUICK_ACTIONS: QuickAction[] = [
  { id: 'home', label: '回零位', icon: <Home className="h-5 w-5" />, command: '回零位待命', className: 'glass-btn px-4 py-2 text-sm font-medium text-white/50' },
  ...
  { id: 'grab', label: '抓取', icon: <Grab className="h-5 w-5" />, command: '闭合夹爪抓取', className: 'glass-btn glass-btn-indigo px-4 py-2 text-sm font-medium text-indigo-300' },
  ...
  { id: 'preset', label: '预设位姿1', icon: <Crosshair className="h-5 w-5" />, command: '移动到预设位姿1', className: 'glass-btn glass-btn-indigo px-4 py-2 text-sm font-medium text-indigo-300' },
];
  ...
  return (
    <div className="space-y-3" role="region" aria-label="快捷操作">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-white/80">快捷操作</h3>
